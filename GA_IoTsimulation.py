...
if (clusterMgrCloudonly):
    ...
    print("Cluster-Resource Manager SF")
...
while MAX_SIMULATION_TIME > sim_time:
    ...
    if len(jobQ) > 0:
        if clusterMgrCloudonly :
            ...
            Cr = GA.returnBESTCRM()            
            # Cr = Chromosome(ClusterRs,jobQ)
            for j in range(len(jobQ)):
                FR_ID = Cr.getRs(j)
                if FR_ID < len(ClusterRs):
                    ClusterRs[FR_ID].ExecutesTask(jobQ[j])
                else:
                    cs.ExecutesTask(jobQ[j])
